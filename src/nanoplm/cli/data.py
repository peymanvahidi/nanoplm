...
def from_yaml(
    ...
    if pretrain_enabled:
        ...
        train_data_dir = _resolve_path(pretrain_config.get("train_data_dir"), cwd)
        val_data_dir = _resolve_path(pretrain_config.get("val_data_dir"), cwd)
        ...
        click.echo(
            "Pretraining shard generation complete:\n"
            f"  Train shards: {len(train_shards)} -> {train_data_dir}\n"
            f"  Val shards:   {len(val_shards)} -> {val_data_dir}"
        )
    else:
